...
aliyun_cmd=("$aliyun_bin" --profile "$profile")

group="$("${aliyun_cmd[@]}" ess DescribeScalingGroups --RegionId "$REGION" --ScalingGroupId.1 "$GATEWAY_SCALING_GROUP_ID")"
...
instances="$("${aliyun_cmd[@]}" ess DescribeScalingInstances --RegionId "$REGION" --ScalingGroupId "$GATEWAY_SCALING_GROUP_ID")"
...
wait_for_count() {
  ...
  for _ in {1..180}; do
    local state count
    state="$("${aliyun_cmd[@]}" ess DescribeScalingInstances --RegionId "$REGION" --ScalingGroupId "$GATEWAY_SCALING_GROUP_ID")"
