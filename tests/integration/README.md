...
`smoke.sh` is intentionally non-mutating and runs after deployment. It verifies that the control API is live and at least three Gateways report ready. The Go test in `internal/dataplane` supplies an authenticated SOCKS5 server and echo target, then verifies a complete `aes-256-gcm` Shadowsocks tunnel and confirms that an upstream route change does not restart the device listener.
